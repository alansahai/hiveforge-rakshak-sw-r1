        ...
        {loading ? (
          ...
          <div className="modal-body" style={{ maxHeight: '72vh', overflowY: 'auto' }}>
            ...
            <div className="panel" style={{ padding: 14, marginBottom: 16, borderLeft: `3px solid ${riskColor}` }}>
              ...
              <div style={{ display: 'flex', gap: 16, marginTop: 8, fontSize: '0.8rem', color: 'var(--text-secondary)', flexWrap: 'wrap' }}>
                <span><strong>Category:</strong> {data.category || 'General Infrastructure'}</span>
                {data.house && <span><strong>House:</strong> {data.house}</span>}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, fontSize: '0.84rem' }}>
                <div>
                  ...
                  <div style={{ fontWeight: 600, color: 'var(--text-primary)', marginTop: 2 }}>
                    {data.contractor || 'State Designated Agency'}
                  </div>
                </div>
                <div>
            ...
            <div className="panel" style={{ padding: 16, marginBottom: 14, border: '1px solid var(--border-card)', background: 'var(--bg-card)' }}>
                ...
                <div style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, marginBottom: 8 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <strong style={{ color: 'var(--risk-critical)', fontSize: '0.92rem' }}>
                    ...
                    {data.geo_duplicate_details?.distance_to_duplicate_km != null && (
                      <span className="risk-badge critical" style={{ fontSize: '0.72rem' }}>
                        {data.geo_duplicate_details.distance_to_duplicate_km} km proximity
                      </span>
                    )}
                  </div>
                  ...
                  {data.geo_duplicate_details?.matched_project_id && (
                    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-subtle)', borderRadius: 4, padding: 12, marginBottom: 10 }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, fontSize: '0.8rem' }}>
                        ...
                        <div style={{ padding: '8px 10px', background: 'rgba(11, 59, 96, 0.06)', borderRadius: 4, border: '1px solid var(--border-subtle)' }}>
                          <div style={{ color: 'var(--accent-primary)', fontWeight: 700, fontSize: '0.74rem', marginBottom: 4 }}>CURRENT PROJECT</div>
                          ...
                          <div style={{ color: 'var(--text-secondary)', fontSize: '0.72rem', marginTop: 2 }}>{data.contractor || 'Designated Agency'}</div>
                        </div>

                        {/* Matched Sibling Project Side */}
                        <div style={{ padding: '8px 10px', background: 'rgba(220, 38, 38, 0.06)', borderRadius: 4, border: '1px solid var(--border-danger)' }}>
                          <div style={{ color: 'var(--risk-critical)', fontWeight: 700, fontSize: '0.74rem', marginBottom: 4 }}>MATCHED SIBLING WORK</div>
                          ...
                          <div style={{ color: 'var(--text-secondary)', fontSize: '0.72rem', marginTop: 2 }}>{data.geo_duplicate_details.matched_contractor || 'Sibling Vendor'}</div>
                        </div>
                      </div>
                      ...
                      <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', marginTop: 10, fontSize: '0.75rem', color: 'var(--text-secondary)' }}>
                        ...
                        <span><strong>Vendor Match:</strong> {data.geo_duplicate_details.same_contractor ? 'Same contractor across administrative border' : 'Different contractors at duplicate GPS proximity'}</span>
                      </div>
                    </div>
                  )}
                </div>
              ...
              <div className="stats-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 8, marginBottom: 14 }}>
                <div style={{ background: 'var(--bg-input)', padding: '10px 12px', borderRadius: 4, border: '1px solid var(--border-subtle)' }}>
                  <div style={{ fontSize: '0.68rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Anomaly Score</div>
                  <div style={{ fontSize: '1.1rem', fontWeight: 700, color: 'var(--accent-primary)', marginTop: 2 }}>
                ...
                <div style={{ background: 'var(--bg-input)', borderRadius: 4, padding: 12, marginBottom: 14, border: '1px solid var(--border-subtle)' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {data.xai_breakdown.primary_risk_drivers?.map((driver, idx) => (
                      <div key={idx} style={{ fontSize: '0.78rem' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
                          <span style={{ fontWeight: 600, color: 'var(--risk-critical)' }}>{driver.feature}</span>
                          <span style={{ fontWeight: 700, color: 'var(--risk-critical)' }}>{driver.impact}</span>
                        </div>
                        ...
                        <div className="progress-bar-container" style={{ height: 4, marginTop: 3 }}>
                          <div className="progress-bar-fill" style={{ width: `${Math.min(driver.score * 2.5, 100)}%`, background: 'var(--risk-critical)' }}></div>
                        </div>
                      </div>
                    ))}
                    {data.xai_breakdown.mitigating_factors?.map((mit, idx) => (
                      <div key={idx} style={{ fontSize: '0.78rem' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
                          <span style={{ fontWeight: 600, color: 'var(--risk-low)' }}>{mit.feature}</span>
                          <span style={{ fontWeight: 700, color: 'var(--risk-low)' }}>{mit.impact}</span>
                        </div>
                        <div style={{ color: 'var(--text-secondary)', fontSize: '0.72rem' }}>{mit.evidence}</div>
                        <div className="progress-bar-container" style={{ height: 4, marginTop: 3 }}>
                          <div className="progress-bar-fill" style={{ width: `${Math.min(Math.abs(mit.score) * 2.5, 100)}%`, background: 'var(--risk-low)' }}></div>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {data.recommended_action && (
                <div style={{
                  padding: '12px 14px',
                  background: (Number(data.fraud_probability) >= 0.6 || riskScore >= 75 || data.geo_duplicate_flag == 1) ? 'rgba(220, 38, 38, 0.08)' : 'rgba(11, 59, 96, 0.06)',
                  border: `1px solid ${(Number(data.fraud_probability) >= 0.6 || riskScore >= 75 || data.geo_duplicate_flag == 1) ? 'var(--border-danger)' : 'var(--border-subtle)'}`,
                  borderRadius: 4,
                  fontSize: '0.84rem'
                }}>
                  <div style={{ color: (Number(data.fraud_probability) >= 0.6 || riskScore >= 75 || data.geo_duplicate_flag == 1) ? 'var(--risk-critical)' : 'var(--accent-primary)', fontWeight: 700, marginBottom: 4 }}>
                    Recommended Vigilance Action &amp; Directives:
                  </div>
